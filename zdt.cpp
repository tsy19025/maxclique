...
void init(){
    ...
    std::memset(nonSucc,0,sizeof(nonSucc));
}
void createGraph(){
    ...
    for(src=0;src<nbVertices;++src){
        ...
        for(dest=0;dest<nbVertices;++dest){
            if(tauE[src][dest]>0){
                succ[src][nbSucc]=dest;
                nbSucc++;
            }else if(src!=dest){
                nonSucc[src][nbNonSucc]=dest; 
                nbNonSucc++;
            }
        }
    }
...
int repair(int* clique,int &cliqueSize){
    ...
    while(1){
        vi = clique[i];
        found = 0;
        for(j=0;((j<nbVertices-degree[vi]-1)&&(found==0));++j){
            vj = nonSucc[vi][j];
            for(k=j+1;((k<nbVertices-degree[vi]-1)&&(found==0));++k){
                vk = nonSucc[vi][k];
                if(isEdge(vj,vk)){
